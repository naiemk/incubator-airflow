...
def incluster_namespace():
    ...
    """
    config.load_incluster_config()
    b64data = config.incluster_config.configuration.api_key['authorization'].split(' ')[-1]
    missing_padding = len(b64data) % 4
    if missing_padding != 0:
        b64data += b'=' * (4 - missing_padding)
    api_key = str(base64.b64decode(b64data))
    return [k for k in api_key.split(',') if 'namespace' in k][0].split(':')[-1].replace('"', '')
